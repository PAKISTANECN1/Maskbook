...
export function useBestV3TradeExactIn(
    ...
    const quotesResults = useMutlipleContractSingleData(quoter, 'quoteExactInput', quoteExactInInputs)
    ...
    return useMemo(() => {
        ...
        const { bestRoute, amountOut } = quotesResults.reduce(
            (
                currentBest: { bestRoute: Route<Currency, Currency> | null; amountOut: BigNumber | null },
                { result },
                i,
            ) => {
...
export function useBestV3TradeExactOut(
    ...
    return useMemo(() => {
        ...
        const { bestRoute, amountIn } = quotesResults.reduce(
            (
                currentBest: { bestRoute: Route<Currency, Currency> | null; amountIn: BigNumber | null },
                { result },
                i,
            ) => {
                if (!result) return currentBest
